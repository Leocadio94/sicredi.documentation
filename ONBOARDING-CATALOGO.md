...
| `oauth-provider` | MISTO | node, react, store | D1 | `/_v/private/sicredi/oauth/{authorize, request-otp, login, token, userinfo, google/*, presession, config}` · `keep-alive` · `audit-logs` · UI de login em `/auth/login` | — | **O gate do site**: Custom OAuth do VTEX ID validando `CA.isSicrediAssociate`; OTP ou Google; não-aprovados entram em pré-sessão (lobby) com login silencioso após aprovação; tem `CLAUDE.md` próprio |
